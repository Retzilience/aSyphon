...
class InputRow(QWidget):
    ...
    def __init__(self) -> None:
        ...
        # Links this row expects to exist when applied "On"
        self._pairs: LinkPairs = []
        # Reconciled reality (updated by reconcile())
        self._actual_connected: bool = False
    ...
    def reconcile(self, backend: PipeWireHubBackend) -> None:
        """
        Update internal "actual connected" based on the real PipeWire graph.
        This prevents false "On" states when the hub or links are removed externally
        (or via hub destroy/create cycles).
        """
        try:
            if self._pairs:
                self._actual_connected = backend.pairs_exist(self._pairs, refresh=False)
            else:
                self._actual_connected = False
        except Exception:
            self._actual_connected = False
        self._sync_ui()
    ...
    def _is_pending(self) -> bool:
        ...
        actual_on = self._actual_connected

        if desired_on != actual_on:
            return True

        if desired_on and actual_on:
    ...
    def apply(self, backend: PipeWireHubBackend) -> bool:
        ...
        sel_key = choice.key
        needs_rewire = (not self._actual_connected) or (self._applied_choice_key != sel_key)
...
class OutputRow(QWidget):
    ...
    def apply(self, backend: PipeWireHubBackend) -> bool:
        ...
        needs_rewire = (not self._actual_connected) or (sink_id != self._applied_sink_id)
        if not needs_rewire:
            self._sync_ui()
            return False
        ...
        try:
            pairs = backend.connect_hub_to_sink(sink_id)
            self._pairs = pairs
            self._actual_connected = True
